...
exports.SpecCriticAgent = void 0;
 ...
 */
class SpecCriticAgent {
    constructor(rlmEndpoint = 'http://localhost:3000') {
        this.id = 'spec-critic';
        this.name = 'Spec Critic';
        this.role = AgentRegistry_1.AgentRole.REVIEWER;
    ...
    async critique(spec) {
        ...
        try {
            ...
            const suggestions = this.generateSuggestions(spec, contradictions, completenessIssues, causalAnalysis);
            // Calculate score
     ...
     */
    async performCausalAnalysis(spec) {
        try {
     ...
     */
    evaluateSpec(spec, contradictions, completenessIssues) {
        const strengths = [];
        const weaknesses = [];
        // Positive: has design
        if (spec.design) {
        ...
        const measurable = spec.requirements.filter(r => /\d+/.test(r) || r.includes('%') || r.includes('ms') || r.includes('seconds'));
     ...
     */
    generateSuggestions(spec, contradictions, completenessIssues, causalAnalysis) {
        const suggestions = [];
     ...
     */
    calculateScore(strengths, weaknesses, contradictions) {
     ...
     */
    determineVerdict(score, contradictions) {
...
exports.SpecCriticAgent = SpecCriticAgent;
//# sourceMappingURL=SpecCriticAgent.js.map