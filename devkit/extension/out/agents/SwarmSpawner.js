...
exports.SwarmSpawner = void 0;
const ModelRouter_1 = require("./ModelRouter");
const WorkerPreamble_1 = require("./WorkerPreamble");
class SwarmSpawner {
    constructor(claude) {
        this.claude = claude;
     ...
     */
    async spawnPipeline(tasks) {
        const pending = [...tasks];
        const allResults = new Map();
        while (pending.length > 0) {
            // Find ready tasks (no pending blockers)
            const ready = pending.filter(task => !task.blockedBy?.some(dep => !allResults.has(dep)));
    ...
    async spawnWorker(task) {
        ...
        const prompt = task.context
            ? WorkerPreamble_1.WorkerPreamble.generateWithContext(task.description, task.context)
            : WorkerPreamble_1.WorkerPreamble.generate(task.description);
        try {
            if (this.claude) {
     ...
     */
    getReadyTasks(tasks) {
        return tasks.filter(task => !task.blockedBy?.some(dep => !this.results.has(dep)));
    }
    /**
...
exports.SwarmSpawner = SwarmSpawner;
//# sourceMappingURL=SwarmSpawner.js.map