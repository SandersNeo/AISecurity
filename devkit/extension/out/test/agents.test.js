...
suite('Agent Interface Test Suite', () => {
    ...
    test('AgentRegistry should have 4 core agents', () => {
        ...
        assert.ok(roles.includes(AgentRegistry_1.AgentRole.PLANNER), 'Should have Planner');
        assert.ok(roles.includes(AgentRegistry_1.AgentRole.CODER), 'Should have Coder');
        assert.ok(roles.includes(AgentRegistry_1.AgentRole.REVIEWER), 'Should have Reviewer');
        assert.ok(roles.includes(AgentRegistry_1.AgentRole.FIXER), 'Should have Fixer');
    });
    test('Agent should have initial IDLE status', () => {
        const registry = new AgentRegistry_1.AgentRegistry();
        const agent = registry.getAgent('planner');
        assert.strictEqual(agent?.status, AgentRegistry_1.AgentStatus.IDLE);
    });
});
suite('Planner Agent Test Suite', () => {
    test('Planner should create plan from spec', async () => {
        const planner = new AgentRegistry_1.PlannerAgent();
        const spec = {
...
suite('Coder Agent Test Suite', () => {
    test('Coder should implement task', async () => {
        const coder = new AgentRegistry_1.CoderAgent();
        const task = {
            id: '1',
...
suite('Reviewer Agent Test Suite', () => {
    test('Reviewer should perform two-stage review', async () => {
        const reviewer = new AgentRegistry_1.ReviewerAgent();
...
suite('Fixer Agent Test Suite', () => {
    test('Fixer should fix issues', async () => {
        const fixer = new AgentRegistry_1.FixerAgent();
        const issues = [
            { id: '1', severity: 'high', description: 'Missing test' }
        ];
...
suite('Agent Orchestration Test Suite', () => {
    test('Should run full workflow: Plan → Code → Review → Fix', async () => {
        ...
        if (review.issues.length > 0) {
            const fixer = registry.getAgent('fixer');
