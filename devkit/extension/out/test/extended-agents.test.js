...
describe('Extended Agents Tests', () => {
    describe('ResearcherAgent', () => {
        it('should parse RLM facts correctly', () => {
            ...
            const facts = response.facts.map((f, index) => ({
                id: f.id || `fact-${index}`,
                content: f.content || '',
                level: `L${f.level}`,
                domain: f.domain,
    ...
    describe('SecurityScannerAgent', () => {
        it('should map engine to threat type correctly', () => {
            const mapEngine = (engine) => {
                ...
                if (e.includes('injection') || e.includes('sqli'))
                    return 'injection';
                if (e.includes('xss'))
                    return 'xss';
        ...
        it('should calculate risk score correctly', () => {
            ...
            const weightedSum = threats.reduce((sum, t) => sum + weights[t.severity] * t.confidence, 0);
    ...
    describe('TesterAgent', () => {
        ...
        it('should generate assertions from requirement', () => {
            ...
            if (requirement.includes('must'))
                assertions.push('mandatory');
            if (requirement.includes('valid'))
                assertions.push('validation');
            if (requirement.includes('error'))
                assertions.push('error handling');
            if (requirement.includes('return'))
                assertions.push('return value');
    ...
    describe('SpecCriticAgent', () => {
        ...
        it('should calculate critique score', () => {
            ...
            score -= contradictions.filter((c) => c.severity === 'major').length * 10; // -10
            assert.strictEqual(score, 65);
        });
        it('should determine verdict correctly', () => {
            const determineVerdict = (score, hasBlocking) => {
                if (hasBlocking)
                    return 'rejected';
                if (score >= 80)
                    return 'approved';
...
//# sourceMappingURL=extended-agents.test.js.map