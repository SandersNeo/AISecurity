...
type AgentType = 'researcher' | 'planner' | 'tester' | 'coder' | 'security' | 'reviewer' | 'fixer' | 'integrator';
...
describe('ModelRouter', () => {
    ...
    describe('getModel', () => {
        ...
        it('should return sonnet for planner, tester, fixer, integrator', () => {
            const sonnetAgents: AgentType[] = ['planner', 'tester', 'fixer', 'integrator'];
...
describe('SwarmSpawner', () => {
    describe('dependency resolution', () => {
        ...
        it('should unblock tasks when dependencies complete', () => {
            ...
            const ready = tasks.filter(t => 
                !t.blockedBy?.some(dep => !completed.has(dep))
            );
    ...
    describe('swarm sizing', () => {
        function estimateSwarmSize(task: string): number {
            if (task.toLowerCase().includes('security') || task.toLowerCase().includes('audit')) return 10;
            if (task.toLowerCase().includes('feature') || task.toLowerCase().includes('implement')) return 4;
            if (task.toLowerCase().includes('fix') || task.toLowerCase().includes('bug')) return 2;
            return 2;
        }
